...
/**
 * Trae el namespace "home" SOLO para el idioma indicado (por defecto el
...
export function registerHomeNamespace(
  ...
  const promise = (async () => {
    try {
      ...
      console.error(
        `[mf-home i18n] no se pudo cargar ${NAMESPACE}/${locale}:`,
        err,
      );
 ...
 */
export function useHomeNamespaceReady(): boolean {
  const [ready, setReady] = useState(false);

  useEffect(() => {
    let cancelled = false;
    setReady(false);
    registerHomeNamespace(i18n.language)
      .then(() => {
